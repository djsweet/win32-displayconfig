"use strict";
const addon = require("bindings")("./win32_monitors");
...
module.exports.extractDisplayConfig = async () => {
    ...
    for (const { value, buffer: pathBuffer } of config.pathArray) {
        ...
        const { modeInfoIdx: sourceModeIdx, adapterId: sourceAdapterId, id: sourceId } = sourceInfo;
        const {
            adapterId,
            id,
            outputTechnology, 
        ...
        const displayNameEntry = config.nameArray.find(n =>
            n.adapterId.LowPart === adapterId.LowPart &&
            n.adapterId.HighPart === adapterId.HighPart &&
            n.id === id &&
            n.outputTechnology && outputTechnology &&
...
module.exports.displayConfigForRestoration = async () => {
    ...
    for (const entry of currentConfig) {
        ...
        const { devicePath, pathBuffer, sourceModeBuffer, targetModeBuffer } = entry;
        ret.push({
            devicePath,
...
async function win32_restoreDisplayConfig(configs, persistent) {
    return new Promise((resolve, reject) => {
        const ran = addon.win32_restoreDisplayConfig(configs, (_, errorCode) => {
            if (errorCode === 0) {
                resolve();
            } else {
                reject(new Win32Error(errorCode));
...
module.exports.restoreDisplayConfig = async (args) => {
    ...
    if (missingEnabled.length === 0) {
        ...
        await module.exports.toggleEnabledDisplays({ enable, disable, persistent: args.persistent });
    }
}
...
addon.win32_listenForDisplayChanges(err => {
    if (err === null) {
        currentDisplayConfigPromise = currentDisplayConfigPromise.then(() => updateDisplayStateAndNotifyCallbacks());
    }
});
